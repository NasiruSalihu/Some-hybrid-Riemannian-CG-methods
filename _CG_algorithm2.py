# -*- coding: utf-8 -*-
"""
Created on Tue Mar 21 08:20:09 2023
...
#from g_linesearch_linesearch import BackTrackingLineSearcher, AdaptiveLineSearcher
from g_linesearch import LineSearchWolfe, AdaptiveLineSearcher
...
def _beta_hager_zhang(
    *,
    ...
    diff = newgrad - oldgrad # yk
    Poldgrad = manifold.transport(x, newx, Pgrad) # gk
    Pdiff = Pnewgrad - Poldgrad # yk+1
    denominator = manifold.inner_product(newx, diff, descent_direction)
    numerator = manifold.inner_product(newx, diff, Pnewgrad)
    numerator -= (
        2* manifold.inner_product(newx, diff, Pdiff)
...
def _HYB1(
  ...
  diff = newgrad - oldgrad
  ip_diff = manifold.inner_product(newx, Pnewgrad, diff)
  beta_fr = newgradPnewgrad / gradPgrad
  beta_pr = max(0, ip_diff / gradPgrad)
  return max(0, min(beta_fr, beta_pr))
...
def _HYB2(
  ...
  diff = newgrad - oldgrad
  denominator= manifold.inner_product(newx, diff, descent_direction)
  beta_dy = newgradPnewgrad / denominator
  try:
      beta_hs = max(
          0,
          manifold.inner_product(newx, Pnewgrad, diff)
      ...
      beta_hs = 1
  return max(0, min(beta_hs, beta_dy))
...
def _HYB3(
  ...
  beta_fr = newgradPnewgrad / gradPgrad
  beta_pr = max(0, ip_diff / gradPgrad)
  return max(-beta_fr, min(beta_fr, beta_pr))
...
def _HYB4(
  ...
  beta_fr = newgradPnewgrad / gradPgrad
  beta_pr = max(0, ip_diff / gradPgrad)
  if 0 <=min(beta_fr,beta_pr): 
      beta_hyb4=min(beta_fr,beta_pr) 
  else:
      beta_hyb4=beta_fr 
  return beta_hyb4
...
def _HRCG1(
    *,
    ...
    descent_direction
     ...
     prpnum = newgradPnewgrad - abs(manifold.inner_product(newx, Pnewgrad, oldgrad))
     denum_1 = 1.25*abs(manifold.inner_product(newx,Pnewgrad,descent_direction))+ gradPgrad
     beta= prpnum / denum_1
     beta_nmu2= max(0, beta)
     beta_fr = newgradPnewgrad / gradPgrad
     return max(beta_nmu2, beta_fr)
...
def _HRVLS1(
    *,
    ...
    descent_direction
     ...
     prpnum = newgradPnewgrad - abs(manifold.inner_product(newx, Pnewgrad, oldgrad))
     denum_1 = 1.25*abs(manifold.inner_product(newx,Pnewgrad,descent_direction))-(manifold.inner_product(x,oldgrad,descent_direction))
     beta= prpnum / denum_1
     beta_vls= max(0, beta)
     diff = newgrad - oldgrad
     ip_diff = manifold.inner_product(newx, Pnewgrad, diff)
     denominator = -manifold.inner_product(x, grad, descent_direction)
     beta_ls = ip_diff / denominator
     return max(0, min(beta_vls, beta_ls))
...
def _HRVLS2(
    *,
    ...
    descent_direction
     ...
     denominator = -manifold.inner_product(x, grad, descent_direction)
     beta_cd = newgradPnewgrad / denominator
     return max(0, min(beta_vls, beta_cd))
...
#BetaTypes = Enum("BetaTypes", "FletcherReeves HagerZhang HestenesStiefel PolakRibiere LiuStorey".split())
...
class ConjugateGradient(Optimizer):
    ...
    Args:
        beta_rule: Conjugate gradient beta rule used to construct the new
            ...
            "PolakRibiere", "HestenesStiefel", "HagerZhang", "NPS", "RMIL"}``.
    ...
    def run(
        ...
        while True:
            ...
            step_size, newx = line_searcher.search(
                objective, manifold, x, descent_direction, cost, df0, gradient
            )
